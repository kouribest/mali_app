...
from mali_project.forms import FormBase, FilterForm
...
class DomainMixin(object):
	...
	def dispatch(self, *args, **kwargs):
		self.domain= self.request.subdomain.name
...
class UpdateForm(DomainMixin, SuccessMessageMixin, UpdateView):
	...
	def get_object(self):
		...
		if obj.etat == 'Valide' and not self.authorized:
			raise PermissionDenied('Vous ne pouvez pas modifier un formulaire déja approuvé')
		return obj
...
class ListRecord(DomainMixin, CustomPermissionMixin, SuccessMessageMixin, ListView):
	...
	def get_queryset(self):
		...
		custom_filter_class = MaliFilter(self.request.GET, queryset= initial_qs) if self.domain == 'Mali' else RdcFilter(self.request.GET, queryset= initial_qs)
	...
	def get_context_data(self, **kwargs):
		...
		context['filter_form'] = self.filter_form if hasattr(self, 'filter_form') else FilterForm()
